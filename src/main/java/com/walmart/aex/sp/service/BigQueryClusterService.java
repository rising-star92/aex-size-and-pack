...
public class BigQueryClusterService {
    ...
    private String generateQuery(String volumeDeviationLevel)  {
        ...
        String queryParams = "WITH\n" +
                "  MyTable AS (\n" +
                "  WITH DATA AS (\n" +
                "    SELECT\n" +
                "      @colors AS json_array ),\n" +
                "    plan_hierarchy AS (\n" +
                "    SELECT\n" +
                "      @planId AS plan_id,\n" +
                ...
                "    SELECT\n" +
                "      JSON_EXTRACT_SCALAR(cc_color_families_json, '$.cc') AS cc,\n" +
    ...
    private String findFinelineQuery(String rfaCcTable, String sizeClusterTable, String sizeColorClusterTable, String finelineClusterTable) {
        return "SELECT\n" +
                ...
                "      FROM (\n" +
                "        SELECT\n" +
                "          RANK() OVER (PARTITION BY store, fineline, cc ORDER BY week ) row_nbr,\n" +
                "          *\n" +
                "        FROM (\n" +
                "          SELECT\n" +
                "            plan_id_partition,\n" +
                ...
                "            plan_hierarchy AS h\n" +
                "          WHERE\n" +
                "            plan_id_partition = h.plan_id\n" +
                "            AND fineline = h.fineline_nbr\n" +
                "            AND final_alloc_space > 0 ) ) AS rfa_cc_output\n" +
                "      WHERE\n" +
                "        rfa_cc_output.row_nbr = 1 ) AS rfa_output\n" +
                "    JOIN (\n" +
                "      SELECT\n" +
                "        svg_fl_clus.dept_nbr,\n" +
                ...
                "        svg_fl_clus.season\n" +
                "      FROM\n" +
                "        `" + finelineClusterTable + "` AS svg_fl_clus,\n" +
                "        plan_hierarchy AS h\n" +
                "      WHERE\n" +
                "        svg_fl_clus.fineline_nbr = h.fineline_nbr\n" +
                "        AND svg_fl_clus.dept_nbr = h.rpt_lvl_1_nbr\n" +
                "        AND svg_fl_clus.season = h.season_code\n" +
                "        AND svg_fl_clus.fiscal_year = h.fiscal_year ) AS fl_clus\n" +
                "    ON\n" +
                "      CAST(rfa_output.store AS INT64) = fl_clus.store_nbr\n" +
                "    LEFT JOIN (\n" +
                "      SELECT\n" +
                "        sc_clus.fineline_nbr,\n" +
                "        sc_clus.dept_nbr,\n" +
                "        sc_clus.store_nbr,\n" +
                "        sc_clus.cluster_id,\n" +
                ...
                "        sc_clus.season\n" +
                "      FROM\n" +
                "        `" + sizeClusterTable + "` AS sc_clus,\n" +
                "        plan_hierarchy AS h\n" +
                "      WHERE\n" +
                "        sc_clus.fineline_nbr = COALESCE(h.like_fineline_nbr, h.fineline_nbr)\n" +
                ...
                "        sco_clus.season\n" +
                "      FROM\n" +
                "        `" + sizeColorClusterTable + "` AS sco_clus,\n" +
                "        plan_hierarchy AS h\n" +
                "      WHERE\n" +
                "        sco_clus.fineline_nbr = COALESCE(h.like_fineline_nbr, h.fineline_nbr)\n" +
                "        AND sco_clus.dept_nbr = COALESCE(h.like_rpt_lvl_1_nbr, h.rpt_lvl_1_nbr)\n" +
                "        AND sco_clus.season = h.season_code\n" +
                "        AND sco_clus.fiscal_year = h.fiscal_year ) AS all_clus\n" +
                "    ON\n" +
                "      CAST(rfa_output.store AS INT64) = all_clus.store_nbr\n" +
                ...
                "        *\n" +
                "      FROM\n" +
                "        cc_color_families ) AS cc_color\n" +
                "    ON\n" +
                "      rfa_output.cc = cc_color.cc\n" +
                "    WHERE\n" +
                "      UPPER(TRIM(all_clus.color_family)) = UPPER(TRIM(cc_color.color_family_desc))\n" +
    ...
    private String findSubCatQuery(String rfaCcTable, String sizeClusterTable, String sizeColorClusterTable, String subCategoryClusterTable) {
        return "SELECT\n" +
                ...
                "      FROM\n" +
                "        `" + subCategoryClusterTable + "` AS svg_subcatg_clus,\n" +
                "        plan_hierarchy AS h\n" +
                "      WHERE\n" +
                "        svg_subcatg_clus.dept_subcatg_nbr = h.rpt_lvl_4_nbr\n" +
                "        AND svg_subcatg_clus.dept_nbr = h.rpt_lvl_1_nbr\n" +
                "        AND svg_subcatg_clus.season = h.season_code\n" +
                "        AND svg_subcatg_clus.fiscal_year = h.fiscal_year ) AS subcatg_clus\n" +
                "    ON\n" +
                "      CAST(rfa_output.store AS INT64) = subcatg_clus.store_nbr\n" +
                "    LEFT JOIN (\n" +
                "      SELECT\n" +
                "        sc_clus.fineline_nbr,\n" +
                "        sc_clus.dept_nbr,\n" +
                ...
                "      CAST(rfa_output.store AS INT64) = all_clus.store_nbr\n" +
                "      AND all_clus.fineline_nbr = COALESCE( rfa_output.like_fineline_nbr, rfa_output.fineline )\n" +
                "      AND all_clus.dept_subcatg_nbr = COALESCE( rfa_output.like_rpt_lvl_4_nbr, rfa_output.rpt_lvl_4_nbr )\n" +
                "      AND rfa_output.fiscal_year = all_clus.fiscal_year\n" +
                "      AND rfa_output.season_code = all_clus.season\n" +
                "    LEFT JOIN (\n" +
                "      SELECT\n" +
    ...
    private String findCatQuery(String rfaCcTable, String sizeClusterTable, String sizeColorClusterTable, String categoryClusterTable) {
        return "SELECT\n" +
                ...
                "      FROM\n" +
                "        `" + categoryClusterTable + "` AS svg_catg_clus,\n" +
                "        plan_hierarchy AS h\n" +
                "      WHERE\n" +
                "        svg_catg_clus.dept_catg_nbr = h.rpt_lvl_3_nbr\n" +
                "        AND svg_catg_clus.dept_nbr = h.rpt_lvl_1_nbr\n" +
                "        AND svg_catg_clus.season = h.season_code\n" +
                "        AND svg_catg_clus.fiscal_year = h.fiscal_year ) AS catg_clus\n" +
                "    ON\n" +
                "      CAST(rfa_output.store AS INT64) = catg_clus.store_nbr\n" +
                "    LEFT JOIN (\n" +
                "      SELECT\n" +
                "        sc_clus.fineline_nbr,\n" +
                "        sc_clus.dept_nbr,\n" +
                ...
                "        plan_hierarchy AS h\n" +
                "      WHERE\n" +
                "        sc_clus.dept_catg_nbr = COALESCE(h.like_rpt_lvl_3_nbr,h.rpt_lvl_3_nbr)\n" +
                ...
                "        plan_hierarchy AS h\n" +
                "      WHERE\n" +
                "        sco_clus.dept_catg_nbr = COALESCE(h.like_rpt_lvl_3_nbr,h.rpt_lvl_3_nbr)\n" +
                ...
                "    ON\n" +
                "      CAST(rfa_output.store AS INT64) = all_clus.store_nbr\n" +
                "      AND all_clus.dept_catg_nbr = COALESCE( rfa_output.like_rpt_lvl_3_nbr, rfa_output.rpt_lvl_3_nbr )\n" +
                "      AND all_clus.fineline_nbr = COALESCE( rfa_output.like_fineline_nbr, rfa_output.fineline )\n" +
                "      AND rfa_output.fiscal_year = all_clus.fiscal_year\n" +
                "      AND rfa_output.season_code = all_clus.season\n" +
                "    LEFT JOIN (\n" +

...
//        assertEquals(1, appMessageTextResponses.stream().filter(appMessageTextResponse -> appMessageTextResponse.getId().equals(200)).map(AppMessageTextResponse::getAppMessageType).map(AppMessageType::getId).findFirst().orElse(null));
//    }
//
//    @Test
